...
# One time step. Register your credentials

fb.app.id <- "1257377700987949" # Your Credentials 
fb.app.secret <- "69b0bb7634271d752af7f4c567c7397e" # Your Credentials 

fb.oauth <- fbOAuth(app_id = fb.app.id, app_secret = fb.app.secret, extended_permissions = TRUE)
save(fb.oauth, file="my.facebook_credentials")

## For  For everyday access, use the following
load(file="my.facebook_credentials")
fb.oauth <- fbOAuth(app_id = fb.app.id, app_secret = fb.app.secret)
...
# Latest Rfacebook Verion 
getFacebookId <- function (name) {
  data <- getUsers(name, token = fb.oauth)
  return (data['id'])
}
data <- getUsers("60844829835", token = fb.oauth)
